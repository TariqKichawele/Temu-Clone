...
export const updateCartItem = async (cartId: string, sanityProductId: string, data: {
    title?: string,
    price?: number,
    quantity?: number,
    image?: string
    ...
    if (existingItem) {
        if (data.quantity === 0) {
            await prisma.cartLineItem.delete({ where: { id: existingItem.id } });
...
export const syncCartWithUser = async (cartId: string | null) => {
    ...
    if (!existingUserCart && !existingAnonymousCart) {
        return createCart();
    }
